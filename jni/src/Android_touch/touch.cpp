		...
		if (read(origfd[i], &ie, sizeof(struct input_event)))
		{
			...
			if (g_Initialized){
            	struct input_event oneevent;                    
       	      	point = rotatePointx(Orientation,(float)Finger[i][latest].x,(float)Finger[i][latest].y,touch_screen_size.x,touch_screen_size.y);
        	    point2.x = point.x / touch_s;point2.y = point.y / touch_s;
				io.MousePos = ImVec2(point.x / touch_s,point.y / touch_s);                            
                if (point2.x <= Pos.x + winWidth && point2.y <= Pos.y + winHeith && point2.x >= Pos.x && point2.y >= Pos.y)
                {
                    Finger[i][latest].isUp = true;
                    Finger[i][latest].isDown = false;
...
void Touch_Init(int *retX, int *retY)
{
	...
	*retY = screenY;
	...
	touch_s = (touch_screen_size.x + touch_screen_size.y) / (full_screen.ScreenX + full_screen.ScreenY);			
}
